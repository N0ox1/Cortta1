...
router.get('/:slug/available-slots', identifyBarbershop, async (req, res) => {
  try {
    ...
    // Verificar se a barbearia funciona na data solicitada
    const requestDate = new Date(date);
    const dayOfWeek = requestDate.getDay();
    const dayNames = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
    
    const workingHours = barbershop.workingHours;
    if (!workingHours) {
      return res.status(400).json({ message: 'Horários de funcionamento não configurados' });
    }
    
    const daySchedule = workingHours[dayNames[dayOfWeek]];
    if (!daySchedule || !daySchedule.isOpen) {
      return res.json([]); // Retorna array vazio se a barbearia não funciona neste dia
    }
    ...
    // Gerar horários disponíveis baseados no horário de funcionamento da barbearia
    ...
    while (currentHour < endHour || (currentHour === endHour && currentMinute < endMinute)) {
      const slotTime = new Date(date + `T${currentHour.toString().padStart(2, '0')}:${currentMinute.toString().padStart(2, '0')}:00`);
...
router.post('/:slug/appointments', identifyBarbershop, async (req, res) => {
  try {
    ...
    // Validar se o horário está dentro do funcionamento da barbearia
    ...
    if (workingHours) {
      ...
      if (!daySchedule || !daySchedule.isOpen) {
        return res.status(400).json({ message: 'A barbearia não funciona neste dia' });
      }
      
      const appointmentTime = appointmentDate.toTimeString().substring(0, 5);
      if (appointmentTime < daySchedule.openTime || appointmentTime >= daySchedule.closeTime) {
        return res.status(400).json({ 
          message: `Horário fora do funcionamento. A barbearia funciona das ${daySchedule.openTime} às ${daySchedule.closeTime}` 
        });
      }
    }
