...
const BarbershopAppointments = () => {
  ...
  useEffect(() => {
    ...
    const slots = [];
    const [startHour] = daySchedule.openTime.split(':').map(Number);
    const [endHour] = daySchedule.closeTime.split(':').map(Number);

    // Incluir desde a hora de abertura até a hora de fechamento (inclusive)
  ...
  const isTimeSlotValid = (timeSlot, selectedDate) => {
    ...
    // Verificar se o slot está antes do horário de fechamento
    if (slotHour < endHour) return true;
    if (slotHour === endHour && slotMinute < endMinute) return true;
