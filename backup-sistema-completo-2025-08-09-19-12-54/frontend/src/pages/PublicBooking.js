...
const PublicBooking = () => {
  ...
  // Horários disponíveis (configuração padrão)
  const workingHours = {
    start: '08:00',
    end: '18:00',
    interval: 30, // minutos
  ...
  const generateTimeSlots = (date) => {
    ...
    startTime.setHours(parseInt(workingHours.start.split(':')[0]), parseInt(workingHours.start.split(':')[1]), 0);
    
    const endTime = new Date(date);
    endTime.setHours(parseInt(workingHours.end.split(':')[0]), parseInt(workingHours.end.split(':')[1]), 0);
    
    let currentTime = new Date(startTime);
    
    while (currentTime < endTime) {
      slots.push(format(currentTime, 'HH:mm'));
      currentTime.setMinutes(currentTime.getMinutes() + workingHours.interval);
    }
    
    return slots;
  };

  // Verificar se a data é válida (não passada e dia útil)
  const isValidDate = (date) => {
    const today = startOfDay(new Date());
    const selectedDateStart = startOfDay(date);
    
    return !isBefore(selectedDateStart, today) && 
           workingHours.workingDays.includes(date.getDay());
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post(`/barbershop/${slug}/appointments`, appointmentData);
