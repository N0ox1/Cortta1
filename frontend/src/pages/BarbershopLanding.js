          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-lg p-6 shadow-sm">
              ...
              <div className="space-y-2 text-gray-600">
                {(() => {
                  const formatWorkingHours = (dayKey) => {
                    const day = barbershop.workingHours?.[dayKey];
                    if (!day) return '08:00 - 18:00';
                    if (!day.isOpen) return 'Fechado';
                    return `${day.openTime} - ${day.closeTime}`;
                  };

                  const formatWeekdays = () => {
                    const monday = barbershop.workingHours?.monday;
                    const friday = barbershop.workingHours?.friday;
                    
                    if (monday?.isOpen && friday?.isOpen && 
                        monday.openTime === friday.openTime && 
                        monday.closeTime === friday.closeTime) {
                  ...
                  return (
                    <>
                      <div className="flex justify-between">
                        ...
                        <span>{formatWeekdays()}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Sábado:</span>
                        <span>{formatWorkingHours('saturday')}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Domingo:</span>
                        <span>{formatWorkingHours('sunday')}</span>
                      </div>
                    </>
                  );
                })()}
