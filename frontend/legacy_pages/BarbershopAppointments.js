...
const BarbershopAppointments = () => {
  ...
  // Carregar horários de funcionamento e barbeiros
  useEffect(() => {
    const fetchBarbershopData = async () => {
      try {
        // Carregar horários de funcionamento
        const profileRes = await api.get('/barbershop/profile');
        const barbershopWorkingHours = profileRes.data.workingHours;
        ...
        // Se for erro de autenticação, não fazer nada
  ...
  // Recarregar dados quando a data mudar (para garantir sincronização)
  useEffect(() => {
    const fetchBarbershopData = async () => {
      try {
        // Recarregar horários de funcionamento para garantir sincronização
        const profileRes = await api.get('/barbershop/profile');
        const barbershopWorkingHours = profileRes.data.workingHours;
        setWorkingHours(barbershopWorkingHours);
      } catch (error) {
        console.error('Erro ao recarregar dados da barbearia:', error);
  ...
  // Gerar timeSlots baseado nos horários de funcionamento
  useEffect(() => {
    ...
    // Gerar slots de hora em hora, começando do horário de abertura até o horário de fechamento (inclusive)
  ...
  // Verificar se um horário específico está dentro do funcionamento
  const isTimeSlotValid = (timeSlot, selectedDate) => {
    if (!workingHours || !selectedDate) return false;
    ...
    // O slot deve estar após o horário de abertura e até o horário de fechamento (inclusive)
        ...
        <div className="bg-white shadow rounded-none overflow-hidden">
          <div className="px-2 sm:px-4 py-5 sm:p-6">
            <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">
              Agenda - {format(new Date(selectedDate + 'T00:00:00'), 'dd/MM/yyyy', { locale: ptBR })}
            </h3>
                    ...
                    {timeSlots.map((timeSlot) => (
                      <tr key={timeSlot} className="hover:bg-gray-50">
                        <td className="sticky left-0 bg-white px-2 sm:px-3 py-0.5 text-xs sm:text-sm font-medium text-gray-900 border-r border-gray-200 w-16 sm:w-20">
                          {isTimeSlotValid(timeSlot.replace(':00', ':30'), selectedDate) ? (
                            // Layout com duas partes (00 e 30)
                            ...
                            // Layout simples apenas com :00 (sem divisória)
                        ...
                        {barbers.map((barber, index) => {
                          ...
                          return (
                            ...
                            >
                              {isTimeSlotValid(`${baseHour}:30`, selectedDate) ? (
                                // Layout com duas partes (00 e 30)
        ...
        {filteredAppointments.length > 0 && (
          <div className="bg-white shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
              <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">
                Agendamentos - {format(new Date(selectedDate + 'T00:00:00'), 'dd/MM/yyyy', { locale: ptBR })}
              </h3>
